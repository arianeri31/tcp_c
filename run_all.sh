...
OUT_SIZE="results_sizes_all.csv"
OUT_SENDS="results_sends_all.csv"

HEADER="server_program,client_program,buffer_size,nb_sends,pool_size,total_sent,total_received,total_elapsed_time_us,total_msg_time_us,avg_msg_time_us,total_notif,fallback_count"

echo "$HEADER" > "$OUT_SIZE"
echo "$HEADER" > "$OUT_SENDS"

echo "======================================"
echo "ZC PING PONG LOOP CSV TESTS WITH PING PONG SERVER"
...
for size in $SIZES; do
    ...
    ./client_zc_loop -s "$size" -n "$FIXED_NB_SENDS" -p "$POOL_SIZE" \
        | grep '^RESULT_COMP' \
        | sed 's/^RESULT_COMP,/server_zc_loop,/' \
        >> "$OUT_SIZE"
done
...
echo "======================================"
echo "ZC PIPELINE LOOP CSV TESTS WITH PING PONG SERVER"
...
echo
echo "======================================"
echo "ZC PING PONG LOOP CSV TESTS WITH PIPELINE SERVER"
echo "Start server_zc_loop before continuing"
echo "........................................"
read -p "Press Enter when server_zc_loop_pipeline is running..."
...
for size in $SIZES; do
    ...
    ./client_zc_loop -s "$size" -n "$FIXED_NB_SENDS" -p "$POOL_SIZE" \
        | grep '^RESULT_COMP' \
        | sed 's/^RESULT_COMP,/server_zc_loop_pipeline,/' \
        >> "$OUT_SIZE"
done
...
echo
echo "======================================"
echo "ZC PIPELINE LOOP CSV TESTS WITH PIPELINE SERVER"
...
echo "======================================"
echo "COPY PING PONG LOOP CSV TESTS WITH PING PONG SERVER"
...
for size in $SIZES; do
    ...
    ./client_copy_loop -s "$size" -n "$FIXED_NB_SENDS" \
        | grep '^RESULT_ALL' \
        | sed 's/^RESULT_ALL,/server_copy_loop,/' \
        >> "$OUT_SIZE"
done
...
echo "======================================"
echo "COPY PIPELINE LOOP CSV TESTS WITH PING PONG SERVER"
...
echo
echo "======================================"
echo "COPY PING PONG LOOP CSV TESTS WITH PIPELINE SERVER"
echo "Start server_copy_loop before continuing"
echo "........................................"
read -p "Press Enter when server_copy_loop_pipeline is running..."
...
for size in $SIZES; do
    ...
    ./client_copy_loop -s "$size" -n "$FIXED_NB_SENDS" \
        | grep '^RESULT_ALL' \
        | sed 's/^RESULT_ALL,/server_copy_loop_pipeline,/' \
        >> "$OUT_SIZE"
done
...
echo
echo "======================================"
echo "COPY PIPELINE LOOP CSV TESTS WITH PIPELINE SERVER"
