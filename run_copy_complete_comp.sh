...
OUT_SIZE="results_sizes_copy_v2_comp.csv"
OUT_SENDS="results_sends_copy_v2_comp.csv"

HEADER="server_program,client_program,buffer_size,nb_sends,total_sent,total_received,total_elapsed_time_us,total_msg_time_us,avg_msg_time_us"
...
for size in $SIZES; do
    ...
    ./client_copy_loop -s "$size" -n "$FIXED_NB_SENDS" \
        ...
        | sed 's/^RESULT_COMP_COPY,/server_copy_loop,/' \
        >> "$OUT_SIZE"
done
...
for size in $SIZES; do
    ...
    ./client_copy_loop -s "$size" -n "$FIXED_NB_SENDS" \
        | grep '^RESULT_COMP_COPY' \
        | sed 's/^RESULT_COMP_COPY,/server_copy_loop_pipeline,/' \
        >> "$OUT_SIZE"
done
